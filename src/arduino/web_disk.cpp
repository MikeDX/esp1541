 ...
 * WiFi AP mode: connect to "ESP1541" network, open http://192.168.4.1
 */
  ...
  <script>
    async function loadSlots() {
      try {
        ...
        div.innerHTML = slots.map((s, i) => 
          '<div class="slot ' + (s.loaded ? 'loaded' : '') + '">' +
          'Slot ' + (i+1) + ': ' + (s.loaded ? s.filename + ' (' + s.size + ' bytes)' : 'empty') +
          '</div>'
        ).join('');
    ...
    document.getElementById('uploadForm').onsubmit = async (e) => {
      ...
      try {
        ...
        document.getElementById('status').textContent = j.ok ? 'Loaded: ' + file.name : 'Error: ' + (j.msg || 'upload failed');
      } catch(err) {
...
void web_disk_setup(void) {
  ...
  server.on("/api/slots", HTTP_GET, [](AsyncWebServerRequest* req) {
    ...
    for (int i = 0; i < RAM_DISK_MAX_SLOTS; i++) {
      ...
      if (s) {
        json += ",\"filename\":\"" + String(s->filename) + "\"";
        json += ",\"size\":" + String(s->size);
        json += ",\"type\":" + String((int)s->type);
      }
      json += "}";
    }
    json += "]";
  ...
  server.on("/upload", HTTP_POST, [](AsyncWebServerRequest* req) {
    // Defer response to body handler
    ...
    if (index + len >= total && upload_buf) {
      ...
      free(upload_buf);
      upload_buf = nullptr;
      req->send(200, "application/json", ok ? "{\"ok\":true}" : "{\"ok\":false,\"msg\":\"no free slot or too large\"}");
    }
  });
...
void web_disk_loop(void) {
  // AsyncWebServer handles requests in background - nothing to poll
  delay(10);
}
