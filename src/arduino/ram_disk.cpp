...
static ram_disk_type_t detect_type(const uint8_t* data, size_t size) {
  ...
  if (size == 174848 || size == 175531) return RAM_DISK_TYPE_D64;  // 35 tracks * 256 * 16 or 17
  ...
  if (size == 174848) return RAM_DISK_TYPE_D64;  // Standard D64
...
bool ram_disk_load(int slot, const uint8_t* data, size_t size, const char* filename) {
  if (slot < 0 || slot >= RAM_DISK_MAX_SLOTS || !data || size > RAM_DISK_MAX_SIZE) {
    return false;
  }
